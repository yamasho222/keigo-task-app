...
export function MinutePicker({
  ...
  const snapToNearest = () => {
    ...
    if (!el) return;
    const idx = Math.round(el.scrollTop / WHEEL_ITEM_H);
    const clamped = Math.min(180, Math.max(1, idx + 1));
    el.scrollTo({ top: (clamped - 1) * WHEEL_ITEM_H, behavior: "smooth" });
    setDraft(clamped);
  };
  ...
  return (
    <>
      <button
        type="button"
        disabled={disabled}
        onClick={() => !disabled && setOpenSafe(true)}
        style={{
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: showStartButton ? 10 : 0 }}>
        <MinutePicker value={duration} onChange={onSetDuration} disabled={disabled} onOpenChange={onPickerOpenChange} />
      </div>
