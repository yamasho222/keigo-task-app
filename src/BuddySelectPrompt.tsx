...
export function BuddySelectPrompt({
  ...
  /** カテゴリ順・各カテゴリ内はレアリティ高い順（アルバムと同じ）。都道府県は地方見出し付き */
  const groups = getAlbumCategoryGroups(stickerAlbum)
    .map((group) => {
      const rewards = group.rewards.filter((r) => owned.has(r.id));
      const subgroups = group.subgroups
        ?.map((sub) => ({
          ...sub,
          rewards: sub.rewards.filter((r) => owned.has(r.id)),
        }))
        .filter((sub) => sub.rewards.length > 0);
      return { ...group, rewards, subgroups };
    })
              ...
              <div key={group.category}>
                ...
                {group.subgroups && group.subgroups.length > 0 ? (
                    ...
                    {group.subgroups.map((sub) => (
                      <div key={sub.id}>
                        ...
                        <div style={{
                          ...
                          {sub.rewards.map(renderBuddyCell)}
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{
                    ...
                    {group.rewards.map(renderBuddyCell)}
                  </div>
                )}
              </div>
            );
